...
export default () => {
  ...
  let orderId: any;
  if (!orderLoading) {
    order[4].parts.map((lastBought: any) => (
  ...
  return (
    <div className='h-screen'>
      <Navbar2 />
      <header className='flex justify-center font-bold md:text-2xl my-4'>Zamówienia użytkownika: {user?.email}</header>
      <div id='partsOverlay' hidden={true}>
        {!partLoading && <PartsOverlay part={part} id={orderId} />}
      </div>
      <div
        className='lg:max-w-[1500px] sm:w-2/3 md:max-w-[675px] m-auto sm:mt-16 relative bg-white overflow-hidden w-screen md:flex wrap md:grid md:grid-cols-4 gap-x-6'>
        <div className='bg-gray-100 justify-center mx-auto sm:col-span-5 w-4/5 mb-2 border-4 border-black  '>
          ...
          <div
            className='text-gray-900 flex flex-row justify-center items-center border-black border-solid border-b'>{variables?.map((variable) => (
            <div key={variable.name} className='w-1/3 flex justify-center m-auto'>{variable.name}</div>))}</div>
              ...
              <div className='flex flex-1 flex-col'>
                <div>
                  <div className='flex text-base font-medium text-gray-900'>
                    {/*Nie działa*/}
                    <p className='flex justify-center w-1/3 ml-2'><a className='truncate'
                                                                     href={`/about/${singleOrder.id}`} onClick={() => {
                    }}>{singleOrder._id}</a></p>
                    <p className='flex justify-center w-1/3' onClick={() => {
                      showItems(false);
                    }}>{singleOrder.parts.length} </p>
